class Player {
  ...
  Player(float x, float y, int id_) {
    ...
    if (id == 0) {
      ...
      c = grey;
      flowersColor = blueRainHighlight;
    } else {
      c = darkGrey;
      flowersColor = blackGrey;
    }
  ...
  void updateTouchControl1Player() {
    acc.set(0, 0);

    // ===================================================== APENAS ANDROID (4) VVV
    ...
    if (touches.length > 0) {
      ...
      accTouchControl.set(averageX-canvas.x-playArea.x - posCenter.x, averageY-canvas.y-playArea.y-em*3 - posCenter.y);
  ...
  void displayTouchControl1Player() {

    // ================================================================== APENAS ANDROID (5) VVV
    if (touches.length == 1) {
      displayTouch(touches[0].x-canvas.x-playArea.x, touches[0].y-canvas.y-playArea.y-em*3);
    }
    //============================================================================== APENAS ANDROID ^^^
  }
  ...
  void autoMove() {
    ...
    for (int i = foods.length - 1; i > 0; i--) {
      if (foods[i].active) {
        ...
        if (foods[i].pos.x > 0 + margin && foods[i].pos.x < playArea.w - 2*margin && foods[i].pos.y > 0 + margin && foods[i].pos.y < playArea.h - 2*margin) {
  ...
  void displayRain() {
    //Chuva
    rectRain(pos.x, pos.y + h/2, w, h*1.5, blueRain, power);
  }
